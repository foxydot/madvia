<?php 
/*
Plugin Name: MSD Custom Facebook Feed
Description: A fork of Custom Facebook Feed by Smash Baloon.
Original Description: Add a completely customizable Facebook feed to your WordPress site
Version: 0.1
Author: MSDLab
...
*/
//Include admin
include dirname( __FILE__ ) .'/msd-custom-facebook-feed-admin.php';
// Add shortcodes
...
function display_cff($atts) {
    ...
    if (!isset($cff_date_position)) $cff_date_position = 'above';
    ...
    if (!$cff_events_only){
        ...
        foreach ($FBdata->data as $news )
        {
            ...
            if ( $cff_show_post ) {
                ...
                $post_text = cff_trim_post($post_text);
                ...
                $cff_link = '<a class="' . $cff_viewpost_class . ' overlay" href="' . $link . '" title="' . $link_text . '" ' . $target . ' ' . $cff_link_styles . '><div class="bug">' . $link_text . '</div></a>';
...
function cff_trim_post($content){
